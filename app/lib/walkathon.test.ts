...
describe("resolveRegistrationCta — per-center override (priority 1)", () => {
  it("uses the override URL when host_own_walk=true and URL is safe", () => {
  ...
  it("ignores the override when host_own_walk=false (falls through to walkathon-level)", () => {
    const override: CenterWalkOverride = {
      hostOwnWalk: false,
  ...
  it("ignores override when URL is null (flag on but URL not yet provided)", () => {
    const override: CenterWalkOverride = {
      hostOwnWalk: true,
  ...
  it("rejects unsafe javascript: URL even with flag on (defense in depth)", () => {
    const override: CenterWalkOverride = {
      hostOwnWalk: true,
